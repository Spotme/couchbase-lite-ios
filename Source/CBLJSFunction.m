                                  ...
                                  JSValueRef* exception)
{
    ...
    NSString* moduleName = (NSString*)CBLJSValueToNSObject/*ValueToID*/(ctx, arguments[0]);
                              ...
                              JSValueRef* exception)
{
    ...
    for (size_t i = 0; i < argumentCount; i++) {
        ...
        id arg = CBLJSValueToNSObject(ctx, argument);
...
- (instancetype) init {
    return [self initWithJSGlobalContextRef: NULL];
}
...
- (JSValueRef) call: (id)param1, ... {
    ...
    jsParams[0] = CBLNSObjectToJSValueRef(context, param1);
    if (_nParams > 1) {
        va_list args;
        va_start(args, param1);
        for (NSUInteger i = 1; i < _nParams; ++i)
            jsParams[i] = CBLNSObjectToJSValueRef(context, va_arg(args, id));
        va_end(args);
    }
    JSValueRef exception = NULL;
...
- (JSValueRef) callWithParams: (NSArray*)params exception: (JSValueRef*)outException {
    ...
    for (NSUInteger idx = 0; idx < params_count; idx++) {
        id obj = params[idx];
        jsParams[idx] = CBLNSObjectToJSValueRef(context, obj);//IDToValue(context, obj);
    }
    JSValueRef exception = NULL;
    JSValueRef result = JSObjectCallAsFunction(context, _fn, NULL, _nParams, jsParams, &exception);
    if (exception) {
...
void WarnJSException(JSContextRef context, NSString* warning, JSValueRef exception) {
    ...
    LogTo(JS, @"Exception: %@: %@", warning, cfError);
    if (cfError)
        CFRelease(cfError);
}

NSString *CBLJSValueToNSString( JSContextRef ctx, JSValueRef v ) {
...
JSValueRef CBLNSObjectToJSValueRef( JSContextRef ctx, NSObject *obj ) {
...
NSObject *CBLJSValueToNSObject( JSContextRef ctx, JSValueRef value ) {
    if (ctx == NULL || value == NULL) { return nil; }
    
	JSType type = JSValueGetType(ctx, value);
	
	switch( type ) {
		case kJSTypeString: return CBLJSValueToNSString(ctx, value);
	...
	if( type == kJSTypeObject ) {
		...
		if( JSValueIsInstanceOfConstructor(ctx, jsObj, arrayConstructor, NULL) ) {
			...
			for( int i = 0; i < count; i++ ) {
				NSObject *obj = CBLJSValueToNSObject(ctx, JSObjectGetPropertyAtIndex(ctx, jsObj, i, NULL));
			...
			for( size_t i = 0; i < count; i++ ) {
				...
				NSObject *obj = CBLJSValueToNSObject(ctx, propValue);
				if (!obj) continue;
